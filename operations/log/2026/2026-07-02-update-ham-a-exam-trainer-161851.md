...
# 更新 Ham-A-Exam-Trainer 静态站点（20260702-161851）

## 目的
本地 Ham-A-Exam-Trainer 又有静态内容更新，需要在 n100 上发布一版新版，同时保留上一版回滚目录。
...
```text
/home/johnson/sites/ham-a-exam-trainer/www.next-20260702-161851
```
...
ssh -o BatchMode=yes n100 \
  'mkdir -p /home/johnson/sites/ham-a-exam-trainer/www.next-20260702-161851'

COPYFILE_DISABLE=1 tar --no-xattrs --exclude .DS_Store --exclude '._*' \
  -C '<local Ham-A-Exam-Trainer>' -cf - . \
  | ssh -o BatchMode=yes n100 \
      'tar -C /home/johnson/sites/ham-a-exam-trainer/www.next-20260702-161851 -xf -'
...
```text
http://192.168.8.15:18080/                    http=200 size=19004  content_type=text/html
http://192.168.8.15:18080/app.js              http=200 size=83722  content_type=application/javascript
http://192.168.8.15:18080/styles.css          http=200 size=26892  content_type=text/css
...
http://192.168.8.15:18080/practice             http=200 size=19004  content_type=text/html
```

NPM 域名验证：

```text
https://ham.jsho.top/                    http=200 size=19004  content_type=text/html
https://ham.jsho.top/app.js              http=200 size=83722  content_type=application/javascript
...
并发 curl 补测中曾出现临时 timeout，容器日志显示对应资源正常返回 `200`；随后使用 `--http1.1 --no-keepalive` 顺序复测通过。
...
ssh -o BatchMode=yes n100 '
  base=/home/johnson/sites/ham-a-exam-trainer
  mv "$base/www" "$base/www.failed-20260702-161851"
  mv "$base/www.backup-20260702-161851" "$base/www"
  docker restart ham-a-exam-trainer
'
```
...
## 影响
- n100 上 `ham-a-exam-trainer` 静态内容已更新到 release `20260702-161851`。
